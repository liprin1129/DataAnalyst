...
# Final Plots
```{r}
levels(pld$LoanStatus)[levels(pld$LoanStatus) == "Past Due (>120 days)"] <- "Dangerous"
levels(pld$LoanStatus)[levels(pld$LoanStatus) == "Past Due (1-15 days)"] <- "Dangerous"
levels(pld$LoanStatus)[levels(pld$LoanStatus) == "Past Due (16-30 days)"] <- "Dangerous"
levels(pld$LoanStatus)[levels(pld$LoanStatus) == "Past Due (31-60 days)"] <- "Dangerous"
levels(pld$LoanStatus)[levels(pld$LoanStatus) == "Past Due (61-90 days)"] <- "Dangerous"
levels(pld$LoanStatus)[levels(pld$LoanStatus) == "Past Due (91-120 days)"] <- "Dangerous"

levels(pld$LoanStatus)[levels(pld$LoanStatus) == "Chargedoff"] <- "Dangerous"
levels(pld$LoanStatus)[levels(pld$LoanStatus) == "Defaulted"] <- "Dangerous"

levels(pld$LoanStatus)[levels(pld$LoanStatus) == "Completed"] <- "Safe"
levels(pld$LoanStatus)[levels(pld$LoanStatus) == "FinalPaymentInProgress"] <- "Safe"

ggplot(aes(x = LoanOriginalAmount, y = MonthlyLoanPayment), data = subset(pld, !is.na(LoanStatus) & 
                                                                            LoanStatus != "Cancelled" &
                                                                            LoanStatus != "Current")) +
  geom_point(aes(colour = LoanStatus), alpha = 0.1, size = 2, position = 'jitter') #+

...
```{r}
ggplot(aes(x = LoanOriginalAmount, y = DebtToIncomeRatio, colour = LoanStatusSimple), data = subset(Monthly, !is.na(LoanStatusSimple) & LoanStatusSimple != "Cancelled" & LoanStatusSimple != "Current")) +
...
Unfortunately, there is not a great difference in the raio of monthly payment to monthly income in original small amount of loan. The gap between two lines become larger as original amount of loan increase. Even though the small gap in small original amount of it, it's obvious people in financial crisis paid back more money for their loan.

```{r}
set.seed(2002)
pld_samp <- pld[sample(1:length(pld$MonthlyLoanPayment), 1000), ]

library(GGally)

ggpairs(pld_samp, #axisLabels = 'internal',
